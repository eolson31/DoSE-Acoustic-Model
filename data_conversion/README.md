...
5. Assuming your path(s) and decoder(s) are correct, you can run the program using `python3 data_conversion/main.py`. This program takes a long time to run, as it converts the entire dataset into trainable Mel spectrograms. However, you should only need to run this once, as the results are ready for the model and are saved to a folder named `train_data`.
